...
BEGIN
  ...
  IF (pType IN ('T', 'B', 'X')) THEN
    ...
    FROM (SELECT COALESCE(ROUND(SUM(taxdetail_tax), 2), 0.0) AS tax
          FROM tax
               JOIN calculateTaxDetailSummary('S', pCoheadid, 'T', COALESCE(pTaxzoneId, -1), pOrderDate, pCurrId, COALESCE(pFreight,0.0)) ON (taxdetail_tax_id=tax_id)
          GROUP BY tax_id) AS data;
  END IF;

  IF (pType IN ('B', 'C')) THEN
    SELECT COALESCE(SUM(currToCurr(aropenalloc_curr_id, pCurrId,
                                   aropenalloc_amount, CURRENT_DATE)),0)
           INTO _credit
    FROM aropenalloc
  ...
  _amount := CASE WHEN pType = 'S' THEN (_subtotal)
                  WHEN pType = 'T' THEN (_subtotal + _tax + COALESCE(pFreight, 0) + COALESCE(pMisc, 0))
                  WHEN pType = 'B' THEN (_subtotal + _tax + COALESCE(pFreight, 0) + COALESCE(pMisc, 0) - _credit)
                  WHEN pType = 'C' THEN (_credit)
                  WHEN pType = 'X' THEN (_tax)
                  WHEN pType = 'M' AND _subtotal != 0.0 THEN (_subtotal - _cost)
                  WHEN pType = 'P' AND _subtotal != 0.0 THEN ((_subtotal - _cost) / _subtotal)
