...
CREATE OR REPLACE FUNCTION calcPurchaseOrderAmt(pPoheadid INTEGER,
                                                pType TEXT, pTaxzoneId INTEGER, pOrderDate DATE, pCurrId INTEGER, pFreight NUMERIC) RETURNS NUMERIC AS $$
-- Copyright (c) 1999-2014 by OpenMFG LLC, d/b/a xTuple. 
-- See www.xtuple.com/CPAL for the full text of the software license.
DECLARE
  _subtotal NUMERIC := 0;
...
BEGIN
  ...
  SELECT COALESCE(SUM(tax), 0) INTO _tax
  FROM ( SELECT COALESCE(ROUND(SUM(taxdetail_tax), 2), 0.0) AS tax
         FROM tax JOIN calculateTaxDetailSummary('PO', pPoheadid, 'T', COALESCE(pTaxzoneId, -1), pOrderDate, pCurrId, COALESCE(pFreight,0.0)) ON (taxdetail_tax_id=tax_id)
         GROUP BY tax_id ) AS data;

  _amount := currToBase(pCurrId,
                        CASE pType WHEN 'S' THEN (_subtotal)
                                   WHEN 'T' THEN (_subtotal + _tax + COALESCE(pFreight, 0) + _freightsub)
                                   WHEN 'X' THEN (_tax)
                                   ELSE 0.0
                        END,
                        pOrderDate);
